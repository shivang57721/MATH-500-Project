...
begin
	# Plot the results
	plot(Ecut_values, eigen_diff_abs[:, 1], label="First Eigenvalue", marker=:circle, yaxis=:log)
	plot!(Ecut_values, eigen_diff_abs[:, 2], label="Second Eigenvalue", marker=:circle, yaxis=:log)
	xlabel!("Ecut")
...
let
	...
	plot(E_cut_values, F_numeric, xlabel="Ecut", ylabel="F with no change in discretisation error", marker=:circle, label="Numeric Calculation")
...
md"""
We compute the first value of $\mathcal{F}$ where the discretisation error is $0$ numerically for various values of $\mathcal{E}$. Part (b) shows that the relationship is $\sqrt{\mathcal{F}} = \sqrt{\mathcal{E}} + \sqrt{\mathcal{E}_V}$. 
Based on the numerical data, we make the estimate $\mathcal{E}_V = 2.256$ by solving for the maximum of $\mathcal{E}_V$ over the various data points.
"""
...
#This cell is used to transform the mother_solver residual output, which has a seperate residual norm for each eigenpaire, into a materix which can be handeled nore easily
begin
...
let n_bands=6, Ecut=7, Ecut_v=2.256, Ecut_large=Ecut+Ecut_v+2*sqrt(Ecut*Ecut_v)
...
### computes λ and ψ at the Γ-point, compute_residual is an option to compute the residual at the cutoff E_cut and at a greater cutoff F
function solver_gamma_point(model, E_cut, n_band::Int, compute_residual::Bool; E_cut_v=2.5)
    # Step 1: Define the PlaneWaveBasis at Γ-point
    basis = PlaneWaveBasis(model; Ecut=E_cut, kgrid=(1, 1, 1))
    ...
    # Step 3: Diagonalize the Hamiltonian for n_band bands (Float64 precision)
    eigres = diagonalize_auto(ham, n_band)
	λ = eigres.eigenvalues[1] # Eigenvalues at the Γ-point
	ψ = eigres.ψ[1]           # Eigenvectors at the Γ-point
	
	if compute_residual
    	# Step 4: Compute residuals at Γ
    	residuals = ham[1] * ψ - ψ * Diagonal(λ)
		residuals_norm = norm.(eachcol(residuals))
		
		#Step 5: Compute residual at Γ on basis_large
		F = E_cut+E_cut_v+2*sqrt(E_cut*E_cut_v) #large cutoff
		#recompute the necessary elements with the large cutoff
		basis_large = PlaneWaveBasis(basis.model; Ecut=F, basis.kgrid)
		ham_large = Hamiltonian(basis_large)
		ψ_large = transfer_blochwave_kpt(ψ, basis, basis.kpoints[1], basis_large, basis_large.kpoints[1])
		#compute the residuals
		residuals_large = ham_large[1] * ψ_large - ψ_large * Diagonal(λ)
		residuals_large_norm = norm.(eachcol(residuals_large))
	else
		#return an array of NaN of the right size
		residuals_norm = [NaN for _ in 1:1:n_band]
		residuals_large_norm = [NaN for _ in 1:1:n_band]
	end


    # Step 7: Return eigenvalues, eigenvectors and residuals' norms
    return (;λ, ψ, residuals_norm, residuals_large_norm)
end

# ╔═╡ 19d4a073-910d-4c83-8e13-b919571dc080
##### Gathering all the data needed #####
begin
	ϵ_range=5:5:30
	iϵ_range = 1:1:size(ϵ_range,1)
	n_band=2

	results = []
	results_double64 = []
	for (iϵ, ϵ) in enumerate(ϵ_range)
		#getting λ, ψ and residuals at the Γ-point
		result_temp = solver_gamma_point(model, ϵ, n_band, true, E_cut_v=2.5)
		push!(results, result_temp)
		#getting λ and ψ in higher precision for the arithmetic error
		result_double64_temp=solver_gamma_point(model, Double64(ϵ), n_band, false)
		push!(results_double64, result_double64_temp)
	end
	#reference at E_cut=80 Ha
	reference = solver_gamma_point(model, 80.0, n_band, false)
end

# ╔═╡ 9ebe0431-e6a9-415c-a3db-243a2d33f85d
let
	#difference with reference:
	error_to_ref = abs.([results[iϵ].λ[1]-reference.λ[1] for iϵ in iϵ_range])
	plot(ϵ_range, error_to_ref, label="error to reference (E_cut=80)", marker=:circle)
	#arithmetic error
	arithmetic_error = abs.([results[iϵ].λ[1]-results_double64[iϵ].λ[1] for iϵ in iϵ_range])
	plot!(ϵ_range, arithmetic_error, label="arithmetic error")
	
	#compute the errors with Bauer Fike:
	#algorithmic error:
	algorithmic_error_bf = [results[iϵ].residuals_norm[1] for iϵ in iϵ_range]
	plot!(ϵ_range, algorithmic_error_bf, label="algorithmic error (BF)", marker=:diamond)
	#algorithmic + discretisation error:
	algo_and_discret_error_bf = [results[iϵ].residuals_large_norm[1] for iϵ in iϵ_range]
	plot!(ϵ_range, algo_and_discret_error_bf-algorithmic_error_bf, label="discretisation error (BF)", marker=:diamond)

	#compute the errors with Kato Temple:
	#algorithmic error:
	gaps = [results[iϵ].λ[2]-results[iϵ].λ[1]-results[iϵ].residuals_norm[1]-results[iϵ].residuals_norm[2] for iϵ in iϵ_range]
	algorithmic_error_kt = [algorithmic_error_bf[iϵ]^2/gaps[iϵ] for iϵ in iϵ_range]
	plot!(ϵ_range, algorithmic_error_kt, label="algorithmic error (KT)", marker=:star)
	#algorithmic + discretisation error:
	gaps_large = [results[iϵ].λ[2]-results[iϵ].λ[1]-results[iϵ].residuals_large_norm[1]-results[iϵ].residuals_large_norm[2] for iϵ in iϵ_range]
	algo_and_discret_error_kt = [algo_and_discret_error_bf[iϵ]^2/gaps_large[iϵ] for iϵ in iϵ_range]
	plot!(ϵ_range, algo_and_discret_error_kt, label="discretisation error (KT)", marker=:star)

	ylabel!("error")
	xlabel!("E_cut (Ha)")
	title!("computation of the errors at Γ-point, n_band=1")
	yaxis!(:log)
end

# ╔═╡ 8c15fa0f-8065-4fa4-bc9a-e2fe9b26c491
md"""
the algorithm and discretisation errors follow the same trend wether they are computed with Bauer Fike or Kato Temple estimates, but the Kato Temple estimate is always around five orders of magnitude better. The arithmetic error is so low that it only becomes relevant at a cutoff higher than 25 Ha.
"""
...
md"""
The Kato-Temple is only available when the eigenvalues are non-degenerated. It can also happen that the gap (computed thanks to the Bauer-Fike estimate) is less than zero (even though the eigenvalues are non-degenerated), making the Kato-Temple bound unavailable. In both case, the error bound defaults to the Bauer-Fike error bound. 
"""
...
let n_bands=6, Ecut=7, Ecut_v=2.256, Ecut_large=Ecut+Ecut_v+2*sqrt(Ecut*Ecut_v)
	...
	for (ik, λk) in enumerate(band_data.eigenvalues)
		...
		gap_temp[1] = λk[2] - λk[1] - eigenvalues_error_BF[ik][1]- eigenvalues_error_BF[ik][2]
		for jn in 2:1:n_bands-1
			gap_temp[jn] = min(λk[jn]-λk[jn-1]-eigenvalues_error_BF[ik][jn]-eigenvalues_error_BF[ik][jn-1], λk[jn+1]-λk[jn]-eigenvalues_error_BF[ik][jn+1]-eigenvalues_error_BF[ik][jn])
		end
		gap_temp[n_bands] = λk[n_bands] - λk[n_bands-1] - eigenvalues_error_BF[ik][n_bands] - eigenvalues_error_BF[ik][n_bands-1]
...
```
Thus, by choosing $\mathcal{F} = \mathcal{E} + \mathcal{E}_V + 2 \sqrt{\mathcal{E} \mathcal{E}_V}$, we obtain that for any $G \in \mathbb{B}_k^{\mathcal{E}}$ and $G' \notin  \mathbb{B}_k^{\mathcal{F}}$ so that $|G' - G| = |ΔG| > \sqrt{2\mathcal{E}_V}$,
```math
...
Then, by linearity of the inner product, the inequality also holds for $x \in X$ with norm $1$.
"""
...
\end{align}
```
where we use that $V_k^{\mathcal{E}^\perp\mathcal{E}} = V_k^{\mathcal{R}\mathcal{E}}$ since $V$ is bounded in the fourier domain and $(V_k^{\mathcal{E}\mathcal{E}^\perp})^H = V_k^{\mathcal{R} \mathcal{E}}$ by the self-adjointness of $V$ and projections.
"""
...
function calculate_lower_bound(model, Ecut, kgrid=(1,1,1))
...
let
	basis = PlaneWaveBasis(model; Ecut=80, kgrid=(1, 1, 1))
    ham = Hamiltonian(basis)
    λ = diagonalize_auto(ham, 2).eigenvalues[1]

	μ1_lower_bounds = []
	μ2_lower_bounds = []
	Ecut_values = 10:5:80
	for Ecut in Ecut_values
		μ1, μ2 = calculate_lower_bound(model, Ecut)
		push!(μ1_lower_bounds, μ1)
		push!(μ2_lower_bounds, μ2)
	end
	plot(Ecut_values, μ1_lower_bounds, marker=:circle, xlabel="Ecut", ylabel="Value", label="μ1 guaranteed lower bound")
	hline!([λ[1]], linestyle=:dash, label="Reference eigenvalue λ1")
	title!("Guaranteed Lower bound for eigenvalue 1")
end
...
# ╔═╡ 6c936e46-be4d-456e-9db6-97c666b6821d
md"""
On a side note, the computation yeilds a result that is way to high, so the actual Shur lower bound is probably closer to the computed eigenvalue compared to what we computed here.
The conclusions that are drawn just above should however stay valid
"""
...
# ╔═╡ f73ca076-ae9a-4cf3-b784-325650c78158
md"""
Estimation of the algorithm and arithmetic error with Double64
"""

# ╔═╡ c9bc2af2-b794-4aaf-9367-432bc54ff84c
begin
	n_bands_D64=6
	Ecut_D64=Double64(7.0)
	#adapt the element type of the model
	model_D64 = convert(Model{Double64}, model)
	#computing the band structure in Double64
	basis_small_D64 = PlaneWaveBasis(model_D64; Ecut=Ecut_D64, kgrid=(1, 1, 1))
	band_data_D64 = compute_bands(basis_small_D64, kpath; kline_density=15, tol=1e-3, n_bands=n_bands_D64)
end

# ╔═╡ f1219215-16a7-47d9-952f-a13997b3b474
let n_bands=6, Ecut=7, Ecut_v=2.256, Ecut_large=Ecut+Ecut_v+2*sqrt(Ecut*Ecut_v)
	#computing the band structure in Float64
	basis_small = PlaneWaveBasis(model; Ecut=Ecut, kgrid=(1, 1, 1))
	band_data = compute_bands(basis_small, kpath; kline_density=15, tol=1e-3, n_bands=n_bands)
	...
	arithmetic_error = [band_data.eigenvalues[ik]-band_data_D64.eigenvalues[ik] for ik in 1:1:size(band_data.eigenvalues, 1)]
	eigenvalues_error = [errork+arithmetic_error[ik] for (ik, errork) in enumerate(eigenvalues_error)]
...
# ╔═╡ 87c447d7-8f42-4b31-add8-526a0ddf3359
md"""
Estimation of the discretisation error using the Shur lower bound
"""

# ╔═╡ e3d58637-5e40-4a10-b8ae-45390c53e609
function calculate_gap(λk, hamk, Ecut, n_bands)
	#computation of l1_V
	Vreal = DFTK.total_local_potential(hamk)
	Vfourier = fft(hamk.basis, hamk.basis.kpoints[1], Complex.(Vreal))
	l1_V = sum(abs, Vfourier)
	#computation of μ_kn
	lower_bounds = []
	for (jn,λkn) in enumerate(λk) 
		b = l1_V - λkn - Ecut
		c = -l1_V^2 + λkn*Ecut - λkn*l1_V
		Δ = b^2 - 4c
		μ1 = 0.5 * (-b - sqrt(Δ))
		#just making sure
		μ2 = 0.5 * (-b + sqrt(Δ))
		...
		push!(lower_bounds, μ1)
	end

	#computation of the gaps
	gaps= zeros(n_bands)
	gaps[1] = lower_bounds[2] - λk[1]
	for jn in 2:1:n_bands-1
		gaps[jn] = min(lower_bounds[jn+1]-λk[jn], lower_bounds[jn]-λk[jn-1])
	end
	gaps[n_bands] = lower_bounds[n_bands] - λk[n_bands-1]

	return gaps	
end

# ╔═╡ 26d8ece1-21bd-41d7-a1aa-86d30954ca36
md"""
As we can see in the result of the cell below, all the gaps computed with the Shur garanteed lower bound are all negative, so they can not be used for the computation of a Kato Temple estimate in practice (or to be more precise, we can not compute μ_kn accurately enough).
"""
...
let n_bands=6, Ecut=7, Ecut_v=2.256, Ecut_large=Ecut+Ecut_v+2*sqrt(Ecut*Ecut_v)
	...
	basis_small = PlaneWaveBasis(model; Ecut=Ecut, kgrid=(1, 1, 1))
	band_data = compute_bands(basis_small, kpath; kline_density=15, tol=1e-3, n_bands=n_bands)
	#setting up the basis for the computation of l1_V
	basis_explicit = PlaneWaveBasis(model; Ecut=Ecut, band_data.basis.kgrid)
	ham_explicit = Hamiltonian(basis_explicit)
	
	gaps_array = []
	for (ik, λk) in enumerate(band_data.eigenvalues)
		#defining the gap
		gap_temp = calculate_gap(λk, ham_explicit[ik], Ecut, n_bands)
		push!(gaps_array, gap_temp)
	end
	gaps_array
end
...
Based on this, if $\| Q_k^\mathcal{E} \widetilde{X}_{kn}\|$ is large relative to $\| P_k^\mathcal{F} \widetilde{r}_{kn}\|$, then the full residual is mostly caused by discretisation error, so we should refine the discretisation to get a more accurate error bound on the eigenvalue.
"""
...
function adaptive_lobpcg(model::Model{T}, Ecut, n_bands;
	...
	Esmall = floor((sqrt(Ecut) - sqrt(2.256))^2) # Using estimate E_V = 2.256
	...
	for i in 1:maxiter
		...
		Q_X = X - P_X
		ratio = maximum(norm.(eachcol(Q_X)) ./ norm.(eachcol(R)))
		
		if (ratio > 1e7) && (Ecut < 80)
			# Change basis
			Ecut = min(Ecut+20, 80)
	...
	# Notice that eigenvalues are returned as λ
	# and eigenvectors as X
	(; λ, X, basis, ham, converged, residual_norms, residual_history, E_values)
end

# ╔═╡ 1d1b7bd7-6673-431e-8802-29cd257d0e69
begin
	res_nonadaptive = nonadaptive_lobpcg(model, 80, 2; tol=1e-8)
	RH = hcat(res_nonadaptive.residual_history...)'
	plot(yaxis=:log, xlabel="Iterations", ylabel="residual")
	plot!(RH[:, 1], marker=:circle, label="EV1 reference")
	plot!(RH[:, 2], marker=:circle, label="EV2 reference")
	
	res_adaptive = adaptive_lobpcg(model, 10, 2; tol=1e-8)
	RH_adaptive = hcat(res_adaptive.residual_history...)'
	plot!(RH_adaptive[:, 1], marker=:circle, label="EV1 adaptive")
	plot!(RH_adaptive[:, 2], marker=:circle, label="EV2 adaptive")
	title!("Residual plot for first two eigenvalues")
	
end

# ╔═╡ bee97b01-7205-4b2c-8ab2-8af285f8b8f9
plot(res_adaptive.E_values, xlabel="Iterations", ylabel="Ecut values", marker=:circle, title="Ecut values for adaptive lobpcg")

# ╔═╡ 87ae5e00-2bb7-433f-8f77-1097fe37b347
md"""
By trial and error, I estimated a good ratio threshold to be $1e7$ and $Δ=20$.
"""
...
manifest_format = "2.0"
project_hash = "ff9f1d3f2cdd7374cfdd8e5f8b8bed07bc20d233"
...
[[deps.DFTK]]
deps = ["AbstractFFTs", "Artifacts", "AtomsBase", "AtomsCalculators", "Brillouin", "ChainRulesCore", "Dates", "DftFunctionals", "DiffResults", "DocStringExtensions", "FFTW", "ForwardDiff", "GPUArraysCore", "Interpolations", "IterTools", "IterativeSolvers", "LazyArtifacts", "Libxc", "LineSearches", "LinearAlgebra", "LinearMaps", "MPI", "Markdown", "Optim", "PeriodicTable", "PkgVersion", "Polynomials", "PrecompileTools", "Preferences", "Primes", "Printf", "PseudoPotentialIO", "Random", "Roots", "SparseArrays", "SpecialFunctions", "Spglib", "StaticArrays", "Statistics", "TimerOutputs", "Unitful", "UnitfulAtomic"]
git-tree-sha1 = "c355fc544e8f3550ec1a1ecb18e950285f5ca797"
...
git-tree-sha1 = "328b9fcde024c2bf17b78816914b0a153b24476a"
...
git-tree-sha1 = "91482b72054f5feadb728cc382141a31e88dad21"
